                                ...
                                <tr>
                                    <td class="ps-4 py-3">
                                        <div class="d-flex align-items-center">
                                            <div class="avatar bg-primary text-white rounded-circle me-3 d-flex align-items-center justify-content-center fw-bold"
                                            ...
                                            <div>
                                                ...
                                                <small
                                                    class="text-muted">{{ optional($mitra->mitraProfile)->company_name ?? 'Perorangan' }}</small>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-3">
                                    ...
                                    <td class="py-3 text-center">
                                        <span
                                            class="fw-bold text-primary fs-5">{{ number_format($mitra->total_points, 0, ',', '.') }}</span>
                                    </td>
                                    <td class="py-3">
                                        @if($mitra->status === 'active')
                                            <span
                                                class="badge bg-success bg-opacity-10 text-success rounded-pill px-3 py-2">Aktif</span>
                                        @elseif($mitra->status === 'pending')
                                            <span
                                                class="badge bg-warning bg-opacity-10 text-warning rounded-pill px-3 py-2">Pending</span>
                                        @else
                                            <span
                                                class="badge bg-danger bg-opacity-10 text-danger rounded-pill px-3 py-2">Suspended</span>
                                        @endif
                                    </td>
                                    <td class="pe-4 py-3 text-end">
                                        <div class="btn-group">
                                            @if($mitra->status === 'pending')
                                                <form action="{{ route('admin.mitras.approve', $mitra) }}" method="POST"
                                                    class="d-inline">
                                                    @csrf
                                            ...
                                            <a href="{{ route('admin.mitras.show', $mitra) }}"
                                                class="btn btn-sm btn-info text-white" title="Lihat Jaringan">
                                                <i class="bi bi-diagram-3"></i>
                                            </a>
                                            <a href="{{ route('admin.mitras.edit', $mitra) }}" class="btn btn-sm btn-warning"
                                                title="Edit Profil">
                                                <i class="bi bi-pencil"></i>
                                            </a>
                                            <form action="{{ route('admin.mitras.destroy', $mitra) }}" method="POST"
                                                onsubmit="return confirm('Suspend mitra ini?');" class="d-inline">
                                                @csrf
                                                @method('DELETE')
