        ...
        @forelse($academicRecords as $index => $record)
            <div class="accordion-item border-0 shadow-sm rounded-4 mb-3 overflow-hidden">
                <h2 class="accordion-header" id="heading{{ $index }}">
                    <button class="accordion-button collapsed bg-white p-4" type="button" data-bs-toggle="collapse"
                                ...
                                <tbody>
                                    @foreach($record->grades as $course)
                                                                <tr class="bg-white border-bottom">
                                                                    <td class="ps-4 py-3 fw-bold text-dark">{{ $course->course_code }}</td>
                                                                    <td class="py-3">{{ $course->course_name }}</td>
                                                                    <td class="py-3 text-center">{{ $course->sks }}</td>
                                                                    <td class="py-3 text-center">
                                                                        @php
                                                                            $badgeClass = 'bg-secondary';
                                                                            $gradePrefix = substr($course->grade_letter, 0, 1);

                                                                            if ($gradePrefix == 'A')
                                                                                $badgeClass = 'bg-success';
                                                                            elseif ($gradePrefix == 'B')
                                                                                $badgeClass = 'bg-primary';
                                                                            elseif ($gradePrefix == 'C')
                                                                                $badgeClass = 'bg-warning text-dark';
                                                                            elseif ($gradePrefix == 'D' || $gradePrefix == 'E')
                                                                                $badgeClass = 'bg-danger';
                                                                        @endphp
                                         <span
                                                                            class="badge {{ $badgeClass }} rounded-pill px-3">{{ $course->grade_letter }}</span>
                                                                    </td>
                                                                    <td class="pe-4 py-3 text-end fw-bold">{{ number_format($course->grade_point, 2) }}</td>
                                                                </tr>
                                    @endforeach
                                </tbody>
                            </table>
