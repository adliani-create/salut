                ...
                <form action="{{ route('mitra.finance.withdraw') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group input-group-lg">
                            ...
                            <input type="number" name="amount" class="form-control bg-light border-start-0 ps-0" placeholder="Minimal 50 poin" min="50" max="{{ Auth::user()->total_points }}" required {{ (!Auth::user()->mitraProfile || !Auth::user()->mitraProfile->bank_number || Auth::user()->total_points < 50) ? 'disabled' : '' }}>
                        </div>
                        <div class="form-text mt-2"><i class="bi bi-info-circle me-1"></i>Min. penarikan 50 poin. Pastikan rekening Anda valid.</div>
                    </div>
                    <button type="submit" class="btn btn-primary w-100 py-3 fw-bold rounded-3" {{ (!Auth::user()->mitraProfile || !Auth::user()->mitraProfile->bank_number || Auth::user()->total_points < 50) ? 'disabled' : '' }}>
