...
<div class="row g-4 mb-4">
    <!-- Peringatan Bank -->
    @if(!Auth::user()->mitraProfile || !Auth::user()->mitraProfile->bank_number)
    <div class="col-12">
        <div class="alert alert-warning border-0 shadow-sm rounded-4 d-flex align-items-center mb-0" role="alert">
            <i class="bi bi-exclamation-triangle-fill fs-3 me-3"></i>
            <div>
                <strong>Perhatian!</strong> Anda belum melengkapi data rekening bank Anda.
                <br>Silakan lengkapi di <a href="{{ route('profile.edit') }}" class="alert-link">Pengaturan Profil</a> agar dapat melakukan penarikan dana.
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="col-md-6">
        <div class="card border-0 shadow-sm rounded-4 h-100">
            ...
            <div class="card-body p-4">
                ...
                <form action="{{ route('mitra.finance.withdraw') }}" method="POST">
                    @csrf
                    <div class="mb-3">
                        ...
                        <div class="input-group input-group-lg">
                            ...
                            <input type="number" name="amount" class="form-control bg-light border-start-0 ps-0" placeholder="Minimal Rp 50.000" min="50000" max="{{ Auth::user()->total_points }}" required {{ (!Auth::user()->mitraProfile || !Auth::user()->mitraProfile->bank_number || Auth::user()->total_points < 50000) ? 'disabled' : '' }}>
                        </div>
                        <div class="form-text mt-2"><i class="bi bi-info-circle me-1"></i>Min. penarikan Rp 50.000. Pastikan rekening Anda valid.</div>
                    </div>
                    <button type="submit" class="btn btn-primary w-100 py-3 fw-bold rounded-3" {{ (!Auth::user()->mitraProfile || !Auth::user()->mitraProfile->bank_number || Auth::user()->total_points < 50000) ? 'disabled' : '' }}>
