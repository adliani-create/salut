    ...
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-header bg-primary text-white border-0 py-3 rounded-top-4">
                <h5 class="fw-bold mb-0"><i class="bi bi-person-plus-fill me-2"></i>Form Input Calon Mahasiswa</h5>
            </div>
            <div class="card-body p-4 p-md-5">
                <p class="text-muted mb-4">Gunakan form ini untuk mencatat data prospek (kandidat mahasiswa baru) yang Anda temui langsung sebelum menyuruh mereka mendaftar via link di HP mereka sendiri.</p>
                ...
                <form action="{{ route('affiliator.students.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        <label class="form-label fw-bold">Asal Sekolah <span class="text-muted">(Opsional)</span></label>
                        <input type="text" name="school_origin" class="form-control form-control-lg @error('school_origin') is-invalid @enderror" value="{{ old('school_origin') }}" placeholder="SMA/SMK/MA sederajat">
                        @error('school_origin') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>

                    <div class="mb-4">
                        <label class="form-label fw-bold">Program Studi yang Diminati <span class="text-muted">(Opsional)</span></label>
                        <!-- Updated to match the existing schema: It takes string or fetched list initially. The user changed this to input earlier -->
